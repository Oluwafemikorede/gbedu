...
public function convertMp3($inputMp3, $bitrate, $outputMp3){

	$convertMp3 = shell_exec('/usr/local/bin/lame  --mp3input  -b '.$bitrate.' '.$inputMp3.' '.$outputMp3.'   2>&1');
	return $convertMp3;
}

public function duration($mp3){
  $duration = shell_exec("/usr/local/bin/ffmpeg -i ".$mp3." 2>&1 | grep Duration | awk '{print $2}' | tr -d ,");
  $exp = explode(':', $duration);
  $duration = $exp[1].':'.$exp[2];
...
function bitRateSampleRate($filename,$type)
{
